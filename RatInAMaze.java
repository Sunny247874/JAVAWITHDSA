...
public class RatInAMaze {
	...
	public static int countPaths(int [][] maze,int n)
	{
		...
		for(int i=0;i<n;i++)
		{
			...
			//if there is blocking then break
			else
				break;
		}
		...
		for(int i=1;i<n;i++)
		{
			...
			//if there is blockage break
			else
				break;
		}
		
		//if maze[i][j]=-1 then ignore otherwise computer the value of count recursively
		for(int i=1;i<n;i++)
		{
			for(int j=1;j<n;j++)
			{
